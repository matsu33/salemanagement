...
function getProductPrice(product) {
	console.log(product);
	var url = 'phieugiaohang/getProductPrice';
	var data = {
...
function clickPrintNewOrder() {
    ...
    if (Object.keys(orderList).length <= 0) {
        ...
        post_to_url('phieugiaohang/excel', dataPost, null);
    }
}
...
function drawListOrder(datatable, orderList) {
    ...
    $.each(orderList, function(key, item) {
        ...
        var row = [image, date_buy, publisher_name, product_name, material_name, diameter, length, product_range, Omss.numberFormat(quanlity), unit_name, linkDelete];
...
function clickSaveNewOrder() {
    if (Object.keys(orderList).length <= 0) {
        ...
        Omss.post('/phieugiaohang/add', dataPost).done(function(data) {
            if (data.status == 1) {
