...
//                    searchCondition.Material = true;
//                    searchCondition.MaterialValue = $(this).find('.material_id').val();
//                    //excuteSearch();
...
//            }
            ...
            if (data) {
                for (var i = 0; i < data.length; i++) {
                    html += '<option value="' + data[i].material_name + '">';
                    html += data[i].id;
                    html += '</option>';
                }
...
function excuteSearch() {
    var url = 'doanhthudauvao/excuteSearch';
            ...
            if (searchCondition.Type == "3") {
                ...
                for (var i = 0; i < obj_group_by.length; i++) {
                    ...
                    for (var j = 0; j < obj_group_by[i].length; j++) {
                        if (obj_group_by[i][j].customer_type == 1) {
                            total_whole_sale = total_whole_sale + Omss.parseFloat(obj_group_by[i][j].total, 0);
                        } else {
                            total_retail_sale = total_retail_sale + Omss.parseFloat(obj_group_by[i][j].total, 0);
                        }
                    }
                    total = total_whole_sale + total_retail_sale;
                    ...
                    var row = [obj_group_by[i][0].year_group_by, Omss.numberFormat(total_whole_sale) + linkDetail, Omss.numberFormat(total_retail_sale), Omss.numberFormat(total)];
                ...
                for (var i = 0; i < obj_group_by.length; i++) {
                    ...
                    var row = [obj_group_by[i][0].date_group_by, Omss.numberFormat(total_whole_sale) + linkDetail, Omss.numberFormat(total_retail_sale), Omss.numberFormat(total)];
                    datatableLoc.fnAddData(row, true);
                }
                datatableLoc.fnDraw();
            }
...
function showDetailType3(date) {
    ...
    for (var i = 0; i < obj_group_by.length; i++) {
        total = 0;
        isShow = false;
        if (obj_group_by[i][0].customer_type == 1) {
            for (var j = 0; j < obj_group_by[i].length; j++) {
            ...
            if(isShow){
            	var row = [date, obj_group_by[i][0].customer_name, Omss.numberFormat(total)];
