...
//    Omss.showSuccess("Tồn kho : " + unitInstock);
    getPriceOfProductThenCallBack(product.id, function(data){
    	var retail_price = data.data[0].retail_price;
    	$('.input_price').autoNumeric('set', retail_price);
    	var input_quanlity = $('.input_quanlity').autoNumeric('get');
    	var total = retail_price * input_quanlity;
...
function drawListOrder(datatable, orderList) {
   ...
   // var publisher_name = $(".order_date_publisher").find(".select_customer").children(':selected').text();
    ...
    $.each(orderList, function(key, item) {
        ...
        var row = [image, date_buy, product_name, material_name, diameter, length, product_range, Omss.numberFormat(quanlity), unit_name, Omss.numberFormat(buy_price), Omss.numberFormat(amount), linkDelete];
...
function clickSaveNewOrder() {
    ...
    if (Object.keys(orderList).length <= 0) {
        ...
        Omss.post('/banle/add', dataPost).done(function(data) {
            console.log(data);
            if (data.status == 1) {
