...
class Controller_Muahang extends Controller_Base
{
	...
	{
		$this->template->title = Lang::get('title_buy');
		$this->template->content = View::forge('muahang/index');
	}
	...
	public function action_add()
	{
		...
		try{
			...
			if (Input::method() == 'POST'){
				...
				$debt = $total;
				$paid = 0;
				$statusOrder = Constant::STATUS_DEBT;
				if($willPaid == 1){
					$debt = 0;
					$paid = $total;
					$statusOrder = Constant::STATUS_PAID;
				}
				$order_type = Constant::ORDER_TYPE_BUY;
	...
	public function getListProductWithGroupId($product_group){
		//init sql
		$querySelect = DB::select($this->tableProducts.".id", $this->tableProductGroup.'.quanlity')
		->from($this->tableProducts)
		->join($this->tableProductGroup,'LEFT')
		->on($this->tableProducts.'.product_group', '=', $this->tableProductGroup.'.id');
	...
	public function action_save()
	{
		...
		$this->template->content = View::forge('muahang/save', $data);
	}
	...
	public function action_print()
	{
		...
		$this->template->content = View::forge('muahang/print', $data);
	}
	...
	public function action_update()
	{
		...
		$this->template->content = View::forge('muahang/update', $data);
	}
	...
	public function action_delete()
	{
		...
		$this->template->content = View::forge('muahang/delete', $data);
	}
