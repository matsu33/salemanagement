...
class Controller_Bansi extends Controller_Base
{
    ...
    {
        $this->template->title   = Lang::get('title_wholesale');
        $this->template->content = View::forge('bansi/index');
    }
    ...
    public function action_add()
    {
        ...
        try {
            ...
            if (Input::method() == 'POST') {
                ...
                $willPaidOrder = Input::param('willPaidOrder');
                $customer_type = Input::param('customer_type') ? Input::param('customer_type') : Constant::TYPE_CUSTOMER_WHOLESALE;
                ...
                $order_type = Constant::ORDER_TYPE_WHOLESALE;
                if($willPaidOrder == 1){
                	$debt = 0;
                	$paid = $total;
                ...
                $dataInsertOrder = array(
                	...
                	'status' => $statusOrder,
                    'date_paid' => $date_buy,
                    'create_at' => $date_buy//date('Y-m-d H:i:s')
                );
                //var_dump($willPaidOrder == 'true');exit;
